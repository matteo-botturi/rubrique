...
public class MenuController {
	...
	@FXML
	private void handleOpenRecentFile(ActionEvent event) {

		if (directoryBean != null && !directoryBean.isSaved()) {
			Boolean shouldSave = handleUnsavedChanges();
			if (shouldSave)
				directoryBean.save();
		}
		...
		if (selectedFile.exists())
			...
			parameterBean.getRecentFiles().remove(selectedFile.getAbsolutePath());
			parameterBean.clearRecentFiles();
			AlertHelper.showError("File Not Found", "The selected file is no longer available.",
	...
	private void updateMenuItems(EMenuState state) {
		...
		case FILE_OPENED ->
			disableItems(true, EMenuItemType.SAVE, EMenuItemType.SAVE_AS, EMenuItemType.CLOSE, EMenuItemType.RELOAD);
		case FILE_SAVED -> disableItems(true, EMenuItemType.SAVE_AS, EMenuItemType.CLOSE, EMenuItemType.RELOAD);
		}
	}

	/**
	 * Handles the action for clearing recent files. Removes all recent files and
	 * updates the menu.
	 */
	 ...
	 */
	private void generateRecentFilesMenu() {
		menuRecentsFiles.getItems().clear();

		for (String filePath : parameterBean.getRecentFiles()) {
			MenuItem fileItem = new MenuItem(filePath);
			fileItem.setOnAction(this::handleOpenRecentFile);
			menuRecentsFiles.getItems().add(fileItem);
		}

		if (!parameterBean.getRecentFiles().isEmpty()) {
	...
	private boolean handleUnsavedChanges() {
		...
		if (directoryBean != null && !directoryBean.isSaved()) {
			Optional<ButtonType> result = AlertHelper.showConfirmation("Unsaved Changes",
					"You have unsaved changes. Do you want to save before proceeding?", ButtonType.YES, ButtonType.NO,
					ButtonType.CANCEL);
